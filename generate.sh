...
echo "Welcome to the GraphQL Profile Page Project README Generator!"
...
read -p "Enter the domain for GraphQL endpoint (replace with your actual domain): " graphqlDomain

# Generate README content
readmeContent=$(cat <<EOL
# ${projectName}

This project is designed to help you learn the GraphQL query language by creating your own profile page. You will interact with the GraphQL endpoint provided by the platform (\`https://${graphqlDomain}/api/graphql-engine/v1/graphql\`). The goal is to query and display your school information on a personalized profile page.
...
1. **Login Page:**
   - Obtain a JSON Web Token (JWT) from the signin endpoint (\`https://${graphqlDomain}/api/auth/signin\`) using Basic authentication.
